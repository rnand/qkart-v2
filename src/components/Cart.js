...
export const generateCartItemsFrom = (cartData, productsData) => {
  ...
  // console.log("---proddata from generateCart",productsData);
  // console.log("---cartdata from generateCart",cartData);
...
const Cart = ({
  ...
  // console.log("--list from cart component",products);
  if (!items.length) {
    return (
  ...
  //   productId,
  //   qty)=>{
  //   // console.log("handleQuantiy-----productId----------------------",productId);

  //   //do the api post call
  ...
  //       console.log("--qty--resp---",qty,resp.data);
  //       setItemQty(!itemQty);
  //       // items = resp.data;
            ...
            {item.qty > 0? (
              <Box display = "flex" alignItems="flex-start" padding="1rem">
                ...
                <Box display="flex" flexDirection="column" justifyContent="space-between" height="6rem" paddingX="1rem">
                  <div>{item.name}</div> <div>${item.cost}</div> <div> Qty: {item.qty}</div>
